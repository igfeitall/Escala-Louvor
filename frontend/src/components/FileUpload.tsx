...
import { formatServicePreview } from '../utils/calendar';
  ...
  return (
    <section className="space-y-5 rounded-[2rem] border border-white/70 bg-white/85 p-6 shadow-panel">
      <div className="space-y-2">
        <h2 className="font-display text-2xl text-ink">Importacao mensal</h2>
        <p className="text-sm text-stone-600">
          Envie um arquivo `.xlsx` ou `.csv`, ou cole o CSV manualmente. Nomes nao
          cadastrados aparecem como aviso.
        </p>
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <label className="flex min-h-52 cursor-pointer flex-col items-center justify-center rounded-[1.75rem] border border-dashed border-accentSoft bg-[linear-gradient(135deg,_rgba(216,184,159,0.18),_rgba(255,255,255,0.9))] p-6 text-center">
          ...
          <span className="mt-2 text-sm text-stone-600">Clique para escolher a planilha do mes.</span>
        </label>
      ...
      >
        {isParsing ? 'Processando...' : 'Ler importacao'}
      </button>

      {parseResult ? (
        <div className="grid gap-4 lg:grid-cols-[2fr_1fr_1fr]">
          <div className="rounded-3xl border border-stone-200 bg-stone-50 p-4">
            <h3 className="font-semibold text-stone-800">Previa</h3>
            <ul className="mt-3 space-y-3 text-sm text-stone-600">
              {parseResult.parsedRows.length === 0 ? (
                <li>Nenhuma linha valida encontrada.</li>
              ) : (
                parseResult.parsedRows.slice(0, 5).map((row) => (
                  <li key={`${row.row}-${row.name}`} className="rounded-2xl bg-white px-3 py-2">
                    ...
                    {row.unavailableServices.length > 0
                      ? row.unavailableServices.map(formatServicePreview).join(', ')
                      : 'sem bloqueios'}
                  </li>
                ))
              )}
          ...
          <div className="rounded-3xl border border-amber-200 bg-amber-50 p-4">
            <h3 className="font-semibold text-amber-900">Nao cadastrados</h3>
